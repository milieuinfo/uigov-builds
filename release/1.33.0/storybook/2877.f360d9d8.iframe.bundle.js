...
`;var m=l("./libs/form/src/next/form-control/form-control.ts"),p=l("./libs/common/utilities/src/index.ts");let v=o.DEFAULT_CLASSNAMES,h=r(),_={AUTO:"auto",TOP:"top",BOTTOM:"bottom"},b={...m.E,options:[],placeholder:"",notDeletable:!1,multiple:!1,search:!1,position:_.AUTO,resultLimit:4,noResultsText:"Geen resultaten gevonden",noChoicesText:"Geen resterende opties gevonden",searchPlaceholder:"Zoek item"};class g extends m.N{static get styles(){return[n.YN,n.Oh,c.K$,d,u,c.oQ,a]}static get properties(){return{options:{type:Array},placeholder:{type:String},notDeletable:{type:Boolean,attribute:"not-deletable"},multiple:{type:Boolean},search:{type:Boolean},position:{type:String},resultLimit:{type:Number,attribute:"result-limit"},noResultsText:{type:String,attribute:"no-results-text"},noChoicesText:{type:String,attribute:"no-choices-text"},searchPlaceholder:{type:String,attribute:"search-placeholder"},value:{type:FormData,state:!0,hasChanged:(e,t)=>e instanceof FormData&&t instanceof FormData?JSON.stringify([...e.entries()])!==JSON.stringify([...t.entries()]):e!==t}}}constructor(){super(),this.options=b.options,this.placeholder=b.placeholder,this.notDeletable=b.notDeletable,this.multiple=b.multiple,this.search=b.search,this.position=b.position,this.resultLimit=b.resultLimit,this.noResultsText=b.noResultsText,this.noChoicesText=b.noChoicesText,this.searchPlaceholder=b.searchPlaceholder,this.value="",this.choices=null,this.initialOptions=[],this.onClickChoices=e=>{e.stopPropagation(),this.disabled||this.choices?.showDropdown()},this.onSearchInput=e=>{let t=e?.target?.value;this.dispatchEvent(new CustomEvent("vl-select-search",{bubbles:!0,composed:!0,detail:{value:t}}))},this.submitFormOnEnter=!1}firstUpdated(e){super.firstUpdated(e),this.choices=new h(this.validationTarget,this.getChoicesConfig()),this.initialOptions=[...JSON.parse(JSON.stringify(this.options))],setTimeout(()=>{this.getChoicesElement()?.addEventListener("click",this.onClickChoices),this.internals.labels[0]?.addEventListener("click",this.onClickChoices),this.value||this.setValue(""),this.choices?.input?.element?.addEventListener("input",this.onSearchInput)},0)}updated(e){if(super.updated(e),this.choices){if(e.has("options")&&(this.choices.clearStore(),this.choices.setChoices(this.getOptions(),"value","label",!0),this.onChange()),e.has("value")){let e={value:this.getSelected()};this.setValue(this.value),this.dispatchEvent(new CustomEvent("vl-select",{bubbles:!0,composed:!0,detail:e})),this.dispatchEventIfValid(e)}e.has("disabled")&&(this.disabled?this.choices.disable():this.choices.enable()),e.has("notDeletable")&&(this.choices.config.removeItemButton=!this.notDeletable,this.choices.config.removeItems=!this.notDeletable),e.has("error")&&this.internals.setValidity({customError:this.error},"custom-error"),e.has("resultLimit")&&(this.choices.config.searchResultLimit=this.resultLimit)}}disconnectedCallback(){super.disconnectedCallback(),this.getChoicesElement()?.removeEventListener("click",this.onClickChoices),this.internals.labels[0]?.removeEventListener("click",this.onClickChoices),this.choices?.input?.element?.removeEventListener("input",this.onSearchInput)}render(){let e={"vl-select":!this.multiple,"vl-multiselect":this.multiple,"vl-select--disabled":this.disabled,"vl-select--error":this.isInvalid||this.error,"vl-select--success":this.success};return(0,i.dy)`
            ...
            ></select>
        `}get validationTarget(){return this.shadowRoot?.querySelector("select")}resetFormControl(){super.resetFormControl(),this.options=[...this.initialOptions]}getSelected(){return this.multiple?this.getSelectedValues():this.getSelectedValues()[0]||""}getSelectedValues(){return Array.from(this.validationTarget.selectedOptions).filter(e=>e.value).map(e=>e.value)}collectFormData(){let e=this.name||this.id,t=this.getSelectedValues();return t?.length?t.reduce((t,l,i)=>(i?t.append(e,l):t.set(e,l),t),new FormData):""}getChoicesElement(){return this.shadowRoot?.querySelector(".js-vl-select")}getChoicesConfig(){return{shouldSort:!1,removeItemButton:!this.notDeletable,removeItems:!this.notDeletable,searchEnabled:this.search,placeholder:!!this.placeholder,placeholderValue:this.placeholder,position:this.position,noResultsText:this.noResultsText,noChoicesText:this.noChoicesText,searchPlaceholderValue:this.searchPlaceholder,classNames:{...v,containerOuter:"js-vl-select",containerInner:"vl-select__inner",input:"vl-input-field",inputCloned:"vl-input-field-cloned",list:"vl-select__list",listItems:"vl-select__list--multiple",listSingle:"vl-select__list--single",listDropdown:"vl-select__list--dropdown",item:"vl-select__item",itemSelectable:"vl-select__item--selectable",itemDisabled:"vl-select__item--disabled",itemChoice:"vl-select__item--choice",placeholder:"vl-select__placeholder",group:"vl-select__group",groupHeading:"vl-select__heading",button:"vl-select__button"},callbackOnCreateTemplates:e=>({containerOuter:()=>e(`<div
